...
<body>
    ...
    <div class="title">
        <b>Reject OT Request</b>
        <form method="post" action="<?= $host_name ?>/home/verify_request_ot">
		    <button type="submit" class="button" style="float:right;">Return</button>
	    </form>
    </div>
    <br>
    <div>
        <form method="post" action="<?= $host_name ?>/home/verify_request_ot_reject">
            <label> Comment Reason for Rejecting Request: </label>
            <br>
            <input type="text" style="border: 1px solid #000000; width:500px; height:42px;" id="reject_reason_id" name="reject_reason" placeholer="Không hợp lý">
            <input type="hidden" name="request_id" value=<?php $_SESSION['cur_ot_request_id'] ?>>
