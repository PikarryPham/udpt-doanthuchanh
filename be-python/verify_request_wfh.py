...
class Request_Verify:
  ...
  def update_status_approved(request_id,status):
    conn = mysql.connect()
    cursor = conn.cursor()
    sqlQuery=('UPDATE `request wfh` SET status=%(_status)s WHERE rwfh_id=%(_request_id)s')
    data_update = {
    '_status' : status,
  ...
  def listall_wfh_request_with_manager(manager_id):
    conn = mysql.connect()
    cursor = conn.cursor()
    sqlQuery=('SELECT * FROM `request wfh` WHERE MANAGER_ID=%(_manager_id)s AND STATUS <>"Draft" AND STATUS <>"Cancelled"')
    data_select = {
...
@app.route('/reject_wfh_request', methods=['PATCH'])
...
@app.route('/approve_wfh_request', methods=['PATCH'])
...
def get_all_wfh_request():
    try:
        _json = request.json
        _manager_id=_json['manager_id']
        if _manager_id and request.method == 'GET':			
            listrequest=Request_Verify.listall_wfh_request_with_manager(_manager_id)
