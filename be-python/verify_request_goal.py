...
app.config['MYSQL_DATABASE_DB'] = 'pagoalform'
app.config['MYSQL_DATABASE_HOST'] = 'pa-goal-test.cizg8kaur6ll.ap-south-1.rds.amazonaws.com'
mysql.init_app(app)
...
class Request_Verify:
  ...
  def update_status_rejected(request_id,status,comment):
    conn = mysql.connect()
    cursor = conn.cursor()
    sqlQuery=('UPDATE pa_goal SET STATUS=%(_status)s, MANAGER_COMMENT=%(_comment)s WHERE PAGOAL_ID=%(_request_id)s')
    data_update = {
    '_status' : status,
    '_comment' : comment,
    '_request_id' : request_id
  ...
  def listall_goal_request_with_manager(manager_id):
    conn = mysql.connect()
    cursor = conn.cursor()
    sqlQuery=('SELECT * FROM pa_goal WHERE MANAGER_ID=%(_manager_id)s AND STATUS <>"Draft" AND STATUS <>"Cancelled"')
    data_select = {
...
@app.route('/reject_goal_request', methods=['PATCH'])
def reject_goal_request():
    try:
        _json = request.json
...
@app.route('/approve_goal_request', methods=['PATCH'])
def approve_goal_request():
    try:
        _json = request.json
        _request_id = _json['request_id']
        _status = _json['status']
...
def get_all_goal_request():
    try:
        _json = request.json
        _manager_id=_json['manager_id']
        if _manager_id and request.method == 'GET':			
            listrequest=Request_Verify.listall_goal_request_with_manager(_manager_id)
