...
app.config['MYSQL_DATABASE_HOST'] = 'leave-request.cizg8kaur6ll.ap-south-1.rds.amazonaws.com'
mysql.init_app(app)
...
class Request_Verify:
  ...
  def update_status_approved(request_id,status):
    conn = mysql.connect()
    cursor = conn.cursor()
    sqlQuery=('UPDATE request_leave SET STATUS=%(_status)s , MANAGER_COMMENT=Null WHERE RLEAVE_ID=%(_request_id)s')
    data_update = {
    '_status' : status,
  ...
  def listall_leave_request_with_manager(manager_id):
    conn = mysql.connect()
    cursor = conn.cursor()
    sqlQuery=('SELECT * FROM request_leave WHERE MANAGER_ID=%(_manager_id)s AND STATUS <>"Draft" AND STATUS <>"Cancelled"')
    data_select = {
